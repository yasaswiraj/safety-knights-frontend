...
export class ConsultantForm3Component implements OnInit {
  ...
  uploadFiles() {
    ...
    // formDataObject['jobDescription'] = this.signUpForm.get('jobDescription')?.value;
  
    // Now send as plain object
    this.formDataService.setFormData({
      formData,
      files: formDataObject
    });
    
    // Debugging
    console.log("Final object sent to setFormData:", formDataObject);
  }
  ...
  //   // No need for fileNamesMap if using FormData with files
  //   return processedData;
  // }
  ...
  private getCategoryValues(categoryName: string): string[] {
    ...
    console.log(`Values for category ${categoryName}:`, values);
  ...
  navigateToNextForm() {
    // Prepare and store FormData with files
    this.uploadFiles();
    this.router.navigate(['/consultant-form8']);
  }
}
